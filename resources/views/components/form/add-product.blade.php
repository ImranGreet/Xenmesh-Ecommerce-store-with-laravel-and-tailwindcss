<div class="w-full lg:w-4/5 mx-auto border rounded-md p-5 shadow lg:shadow-lg shadow-gray-800/60">
    <div class="text-center">
        <h1 class="text-blue-800 text-xl lg:text-3xl tracking-wide">Insert New Product</h1>
    </div>
    ...
    <form action="{{route('add_product')}}" method="post" enctype="multipart/form-data">
        ...
        <div class="w-full flex flex-col space-y-2">
          ...
          <input type="text" name="title" value="{{old('title')}}"  placeholder="Enter Your Product Title" class="w-full px-4 py-2 border border-gray-700 rounded-md focus:outline-none focus:ring-2 ring-blue-500">
          @error('title')
       ...
       <div class="flex-col flex xl:flex-row lg:space-x-3">
          <div class="w-full flex flex-col space-y-2">
              ...
              <input type="text" name="price"  placeholder="$12.50" value="{{old('price')}}" class="w-full px-4 py-2 border border-gray-700 rounded-md focus:outline-none focus:ring-2 ring-blue-500">
              @error('price')
                <span class="text-red-400 text-sm italic" role="alert">
                <strong>{{ $message }}</strong>
                </span>
       ...
       <div class="w-full flex flex-col space-y-2">
        ...
        <div class="w-full flex flex-col space-y-2">
          ...
          <textarea id="message" rows="6" name="description" value="{{old('description')}}" class="w-full px-4 py-2 border border-gray-700 rounded-md focus:outline-none focus:ring-2 ring-blue-500" placeholder="Write your description here..."></textarea>
          @error('description')
            <span class="text-red-400 text-sm italic" role="alert">
                <strong>{{ $message }}</strong>
            </span>
